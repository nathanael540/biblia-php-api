...
use solion\core\Loader;
use solion\core\Dispatcher;
 ...
 * @method \solion\template\View view() Gets current view
 *
 * Request-response
...
class Engine
{
    ...
    public function init()
    {
        ...
        $this->loader->register('request', '\solion\net\Request');
        $this->loader->register('response', '\solion\net\Response');
        $this->loader->register('router', '\solion\net\Router');
        $this->loader->register('view', '\solion\template\View', array(), function ($view) use ($self) {
            $view->path = $self->get('solion.views.path');
            $view->extension = $self->get('solion.views.extension');
        });
        ...
        $this->set('solion.base_url', null);
        $this->set('solion.case_sensitive', false);
        $this->set('solion.handle_errors', true);
        $this->set('solion.log_errors', false);
        $this->set('solion.views.path', './views');
        $this->set('solion.views.extension', '.php');
        $this->set('solion.content_length', true);

        // Startup configuration
        $this->before('start', function () use ($self) {
            // Enable error handling
            if ($self->get('solion.handle_errors')) {
                set_error_handler(array($self, 'handleError'));
                set_exception_handler(array($self, 'handleException'));
            }

            // Load our modules from custom folder
            $modulesDIR = dirname(__FILE__) . "/modules";
            $modules = glob($modulesDIR . '/*.{php}', GLOB_BRACE);
            foreach ($modules as $module) {
                require $module;
            }

            // Set case-sensitivity
            $self->router()->case_sensitive = $self->get('solion.case_sensitive');
            // Set Content-Length
            $self->response()->content_length = $self->get('solion.content_length');
        });
